...
public class EasyController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        while (iter > 0)
        {
            ...
            DroneColorCmd(bufferList[0].ToString());
    ...
    // Drone Color
    private void DroneColorCmd(string msg)
    {
        string[] cmd = msg.Split(' ');
        if (cmd.Length >= 2)
        {
            string stringValue = System.Text.RegularExpressions.Regex.Match(cmd[1], @"\d+").Value;

            float value = float.Parse(stringValue) / 127.0f;
            if (cmd[0] == "1")
            {
                H = value;
            }
